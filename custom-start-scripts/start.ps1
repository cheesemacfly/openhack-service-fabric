...
try {
#########################
# Start global try/catch
#########################

if (!(Test-Path -Path '/data/eula.txt')) {
  if ($Env:EULA -eq 'TRUE') {
    [System.IO.File]::WriteAllText("Z:\eula.txt", "# Generated via Docker on $(Get-Date)`neula=TRUE")
  } else {
    echo ""
...
{
  "X|XLATEST|Xlatest" {
    $VANILLA_VERSION = ($http.GetStringAsync($VERSIONS_JSON).GetAwaiter().GetResult() | convertfrom-json | select -expand latest).release
    break
  }
  "XSNAPSHOT|Xsnapshot" {
    $VANILLA_VERSION = ($http.GetStringAsync($VERSIONS_JSON).GetAwaiter().GetResult() | convertfrom-json | select -expand latest).snapshot
    break
  }
  "X[1-9]*" {
...
function setServerProp($prop, $val) {
  if ($val) {
    ...
    [System.IO.File]::WriteAllText('Z:\server.properties', ((Get-Content -Raw .\server.properties) | % { $_ -Replace "$prop=(.*)`n", "$prop=$val`n" }))
  }
}
